...
// Top-level tasks only. A step with six subtasks is still one step, and a
// tally that counted them would swing on the size of a step rather than on
...
// One row: a dim gutter, the status icon in the task's colour, then the text
// wrapped and lined up under itself. `gap` is what sits between the icon and
// the text, which is how a subtask row is indented one step further in.
function rowLines(task, gutter, gap, width, config) {
  ...
  const indent = ' '.repeat(cellWidth(gutter) + cellWidth(icon) + gap.length);
  const room = Math.max(8, width - indent.length);
  const body = wrap(text, room);
  ...
  const head = `${DIM}${gutter}${RESET}${colour}${icon}${RESET}${gap}${colour}${weight}${body[0]}${RESET}`;
  const lines = [head, ...body.slice(1).map((line) => `${indent}${colour}${weight}${line}${RESET}`)];
  if (task.note) {
    for (const line of wrap(`↳ ${task.note}`, room)) lines.push(`${indent}${DIM}${line}${RESET}`);
  }
  return lines;
}

// One task becomes one or more display lines, already styled, with its
// subtasks in the same block so the scroll window never separates them.
function taskLines(task, index, width, config) {
  const number = `${String(index + 1).padStart(2, ' ')} `;
  const lines = rowLines(task, number, '  ', width, config);
  const subIndent = ' '.repeat(number.length + 3);
  const subtasks = config.show_completed
    ? task.subtasks || []
    : (task.subtasks || []).filter((sub) => sub.status !== 'completed');
  for (const sub of subtasks) lines.push(...rowLines(sub, subIndent, ' ', width, config));
  return lines;
}
...
function render(record, size, config, options = {}) {
  ...
  // The footer is always two lines, whatever it ends up saying, so the room for
  // tasks can be worked out before the text is known.
  const showFooter = options.footer !== false;
  const room = height - lines.length - (showFooter ? 2 : 0);
  const footer = (hint) => (showFooter ? ['', `${DIM}${hint}${RESET}`] : []);

  if (!total) {
    lines.push(`${DIM}Waiting for the agent's task list…${RESET}`);
    return { lines: lines.concat(footer('q quit')), offset: 0 };
  }

  const blocks = visible.map((task, i) => taskLines(task, record.tasks.indexOf(task), width - 1, config));
  const activeBlock = visible.findIndex(
    (t) => t.status === 'in_progress' || (t.subtasks || []).some((s) => s.status === 'in_progress'),
  );
  const { offset } =
  ...
  // Only offer the scroll keys when there is something below the fold. A list
  // that fits has nothing for j/k/g to do, and a hint for a key that does
  // nothing reads as a broken key. `f` is offered once the view is pinned, which
  // is the only moment it means anything.
  const maxOffset = Math.max(0, flat.length - room);
  const hint = !maxOffset
    ? 'q quit'
    : options.offset === undefined
      ? 'q quit · j/k scroll · g top · G end'
      : 'q quit · j/k scroll · g top · G end · f follow';

  return { lines: lines.concat(footer(hint)), maxOffset, offset: clamped };
}
